...
typedef struct flowie_security_turbodb_loaded_s {
  vec_t rules;
} flowie_security_turbodb_loaded_t;

struct flowie_security_turbodb_provider_s {
  tstr database_path;
  tstr namespace_name;
  int busy_timeout_ms;
  size_t max_rules;
...
static int flowie_security_orm_query_visit(orm_connection_t *connection,
                                           orm_transaction_t *transaction, const char *sql,
                                           const char *parameter,
                                           const flowie_orm_column_t *columns, size_t column_count,
                                           size_t max_rows, flowie_orm_row_visit_fn visit,
                                           void *visit_ctx, size_t *row_count) {
  orm_error_t error;
  ...
  if (rc == TURBO_OK)
    rc = flowie_orm_query_visit(query, transaction, columns, column_count, max_rows, 0u, visit,
                                visit_ctx, row_count);
  orm_query_destroy(query);
  return rc;
}

static int flowie_security_orm_open(const flowie_security_turbodb_provider_t *provider,
  ...
  orm_config(&config);
  if (snprintf(timeout, sizeof(timeout), "%d", provider->busy_timeout_ms) <= 0) return TURBO_EINVAL;
...
static void flowie_security_turbodb_loaded_destroy(flowie_security_turbodb_loaded_t *loaded) {
  if (!loaded) return;
...
typedef struct flowie_security_rules_load_s {
  const flowie_security_turbodb_provider_t *provider;
  vec_t *rules;
} flowie_security_rules_load_t;

static int flowie_security_rule_visit(void *ctx, const flowie_orm_row_t *row, size_t row_index) {
  ...
  if (!load || !load->provider || !load->rules || row_index != vec_size(load->rules) ||
      ordinal != row_index || !line || line_size == 0u ||
      line_size > FLOWIE_SECURITY_RULE_LINE_MAX || memchr(line, '\0', line_size))
    return TURBO_EPROTO;
...
static int flowie_security_turbodb_load(void *ctx, uint64_t required_version,
                                        flowie_security_policy_bundle_t *bundle_out) {
  static const char metadata_sql[] = "select policy_version,expires_at from "
                                     "flowie_control_published_bundle where namespace_name=?";
  static const char rules_sql[] = "select ordinal,rule_line from flowie_control_published_rule "
                                  "where namespace_name=? order by ordinal";
  flowie_security_turbodb_provider_t *provider = (flowie_security_turbodb_provider_t *)ctx;
  flowie_security_turbodb_loaded_t *loaded = NULL;
  ...
  *bundle_out = (flowie_security_policy_bundle_t)FLOWIE_SECURITY_POLICY_BUNDLE_INIT;
  ...
  if (rc == TURBO_OK)
    rc = flowie_security_orm_status(
        orm_transaction_begin(connection, ORM_ISOLATION_SERIALIZABLE, &transaction, &error));
  if (rc == TURBO_OK)
    rc = flowie_security_orm_query_visit(connection, transaction, metadata_sql,
                                         provider->namespace_name, metadata_columns,
  ...
  if (rc == TURBO_OK && (metadata.policy_version == 0u ||
                         (required_version != 0u && required_version != metadata.policy_version)))
    rc = TURBO_ENOENT;
  if (rc == TURBO_OK) {
    loaded = (flowie_security_turbodb_loaded_t *)calloc(1u, sizeof(*loaded));
    if (!loaded) rc = TURBO_ENOMEM;
  }
  if (rc == TURBO_OK) {
    rc = flowie_security_stl_status(vec_init_bytes(&loaded->rules, sizeof(flowie_security_rule_t),
                                                   _Alignof(flowie_security_rule_t),
                                                   provider->max_rules));
  }
  ...
  if (rc == TURBO_OK) rc = flowie_security_orm_status(orm_transaction_commit(transaction, &error));
  else if (transaction) (void)orm_transaction_rollback(transaction, &error);
  if (rc == TURBO_OK) {
  ...
  flowie_security_turbodb_loaded_destroy(loaded);
  return rc;
}

static void flowie_security_turbodb_release(void *ctx, flowie_security_policy_bundle_t *bundle) {
  (void)ctx;
  if (!bundle) return;
  flowie_security_turbodb_loaded_destroy(
      (flowie_security_turbodb_loaded_t *)bundle->provider_bundle);
...
int flowie_security_turbodb_provider_create(const flowie_security_turbodb_config_t *config,
  ...
  flowie_security_turbodb_provider_t *provider;
  orm_connection_t *probe = NULL;
  int rc;
  if (out) *out = NULL;
  if (!config || config->size < sizeof(*config) ||
      config->api_version != FLOWIE_SECURITY_TURBODB_API_VERSION || !config->database_path ||
      ...
      !out)
    return TURBO_EINVAL;
  provider = (flowie_security_turbodb_provider_t *)calloc(1u, sizeof(*provider));
  ...
  provider->interface.load = flowie_security_turbodb_load;
  provider->interface.release = flowie_security_turbodb_release;
  *out = provider;
  return TURBO_OK;
}
