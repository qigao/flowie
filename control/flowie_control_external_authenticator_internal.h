...
typedef enum flowie_control_external_authenticator_capability_e {
  ...
  FLOWIE_CONTROL_EXTERNAL_AUTH_DOMAIN_BINDING = 1u << 4
...
#define FLOWIE_CONTROL_EXTERNAL_AUTH_REQUIRED_CAPABILITIES                                         \
  (FLOWIE_CONTROL_EXTERNAL_AUTH_STABLE_SUBJECT | FLOWIE_CONTROL_EXTERNAL_AUTH_ACCOUNT_STATE |      \
   FLOWIE_CONTROL_EXTERNAL_AUTH_EXPIRING_ASSERTION | FLOWIE_CONTROL_EXTERNAL_AUTH_DOMAIN_BINDING)
 ...
 *
 * All pointers are borrowed only for the duration of verify(). domain_id is an optional expected
 * Domain constraint: an empty string requests trusted Domain discovery from the assertion. The
 * secret is the presented password/token, never a stored verifier. A network-backed callback may
 * yield in the current coroutine; a blocking SDK must be isolated behind a bounded executor by its
 * adapter.
 */
 ...
 * the repository subsequently checks the principal in the assertion-bound Domain and loads only
 * local roles/groups. A Broker service principal's Domain is not a substitute for this binding.
 */
