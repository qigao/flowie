...
CXX_C_API int flowie_bitmap_index_create(size_t max_members,
                                             flowie_bitmap_index_t **out);
CXX_C_API void flowie_bitmap_index_destroy(flowie_bitmap_index_t *index);

/** Idempotently add/remove one integer member. */
CXX_C_API int flowie_bitmap_index_add(flowie_bitmap_index_t *index, uint64_t member);
CXX_C_API int flowie_bitmap_index_remove(flowie_bitmap_index_t *index, uint64_t member);

CXX_C_API int flowie_bitmap_index_contains(const flowie_bitmap_index_t *index,
                                                uint64_t member, int *out);
CXX_C_API int flowie_bitmap_index_count(const flowie_bitmap_index_t *index, size_t *out);

/** Select the zero-based member rank in ascending order; an invalid rank returns TURBO_ERANGE. */
CXX_C_API int flowie_bitmap_index_select(const flowie_bitmap_index_t *index, size_t rank,
                                              uint64_t *out);
