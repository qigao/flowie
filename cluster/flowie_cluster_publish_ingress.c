...
static void flowie_cluster_publish_ingress_remove(
  ...
  if (flowie_stl_error(vec_swap_remove(&ingress->entries, index, &removed)) == TURBO_OK)
...
static int flowie_cluster_publish_ingress_send_ack(
  ...
  if (rc == TURBO_OK) entry->ack_pending = 0;
  return rc;
}
...
int flowie_cluster_publish_ingress_create(
  ...
  if (!ingress) return TURBO_ENOMEM;
...
int flowie_cluster_publish_ingress_handle(
  ...
  if (entry && entry->ack_pending) {
    ...
    if (chunk->stream_offset < acknowledged_offset) return TURBO_OK;
  }
  if (!entry) {
