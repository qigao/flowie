...
static void flowie_raft_store_cleanup(const char *path) {
  char generated[SALTS_FS_MAX_PATH];
  size_t segment;
  if (!path) return;
...
static int flowie_raft_store_apply(void *ctx, const tr_raft_entry_t *entries,
  ...
  size_t index;
  if (!capture || !entries || entry_count == 0u) return SALTS_EINVAL;
  for (index = 0u; index < entry_count; ++index) {
    if (entries[index].data_length > sizeof(capture->data)) return SALTS_EMSGSIZE;
