...
spec("flowie cluster protocol kernel") {
  group("configuration") {
    it("accepts bounded lease and queue limits") {
      ...
      check_equal(flowie_cluster_config_validate(&config), TURBO_OK);
    }

    it("rejects a renewal budget that reaches the lease ttl") {
      flowie_cluster_config_t config = flowie_cluster_test_config();
      config.renew_interval_ms = 13000u;
      check_equal(flowie_cluster_config_validate(&config), TURBO_EINVAL);
      config.renew_interval_ms = UINT64_MAX;
      check_equal(flowie_cluster_config_validate(&config), TURBO_ERANGE);
    }
  ...
  group("local runtime adapter") {
    it("assigns every key to one immutable local owner") {
      ...
      check_equal(flowie_cluster_runtime_create_local(7u, &runtime), TURBO_OK);
      ...
      check_equal(flowie_cluster_owner_token_require(&session_owner, &retained_owner), TURBO_OK);
    ...
    it("rejects invalid creation and key inputs without leaking an output token") {
      ...
      check_equal(flowie_cluster_runtime_create_local(0u, &runtime), TURBO_EINVAL);
      check_null(runtime);
      check_equal(flowie_cluster_runtime_create_local(8u, &runtime), TURBO_OK);
  ...
  group("lease and capacity arithmetic") {
    ...
    it("rejects expired and overflowing lease deadlines") {
      ...
      check_equal(flowie_cluster_lease_deadline_ns(0u, 1000u, 1000u, &deadline), TURBO_EINVAL);
    ...
    it("calculates queue entries with ceiling and checked arithmetic") {
      uint64_t entries = 0u;
      check_equal(flowie_cluster_required_queue_entries(1501u, 250u, 32u, &entries), TURBO_OK);
  ...
  group("fencing token") {
    it("accepts only an exact node boot shard and epoch match") {
      ...
      check_equal(flowie_cluster_owner_token_init(&expected, 7u, 42u, "node-a", 6u, boot_id),
                   TURBO_OK);
      presented = expected;
      check_equal(flowie_cluster_owner_token_require(&expected, &presented), TURBO_OK);
      presented.owner_epoch = 41u;
      check_equal(flowie_cluster_owner_token_require(&expected, &presented), TURBO_EBUSY);
      presented = expected;
    ...
    it("rejects malformed tokens before comparing authority") {
      ...
      check_equal(flowie_cluster_owner_token_init(&token, 1u, 1u, "node", 4u, zero_boot),
                   TURBO_EINVAL);
      check_equal(flowie_cluster_owner_token_require(&token, &token), TURBO_EINVAL);
    }
  }
  ...
  group("authoritative shard lease") {
    it("claims renews authorizes and releases one epoch") {
      ...
      check_equal(flowie_cluster_shard_lease_require(&lease, 5999u, &token), TURBO_OK);
      check_equal(flowie_cluster_shard_lease_renew(&lease, 2000u, 5000u, &token, &lease_until),
                   TURBO_OK);
      check_equal(lease_until, 7000u);
      check_equal(flowie_cluster_shard_lease_release(&lease, 3000u, &token), TURBO_OK);
    ...
    it("fences stale owners after expiry and epoch advancement") {
      ...
      check_equal(flowie_cluster_shard_lease_require(&lease, 151u, &stale), TURBO_EBUSY);
      check_equal(flowie_cluster_shard_lease_release(&lease, 151u, &stale), TURBO_EBUSY);
      check_equal(flowie_cluster_shard_lease_require(&lease, 151u, &current), TURBO_OK);
    }
