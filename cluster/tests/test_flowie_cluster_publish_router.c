...
static int flowie_publish_router_commit(
  ...
  (void)ctx;
  return event ? TURBO_OK : TURBO_EINVAL;
}
...
static int flowie_publish_router_propose(void *ctx,
  ...
  flowie_publish_router_test_t *test =
      (flowie_publish_router_test_t *)ctx;
  if (!test || !proposal) return TURBO_EINVAL;
  ++test->proposal_count;
...
static tr_raft_coronet_payload_t flowie_publish_router_ack(
...
spec("flowie cluster publish router") {
  it("submits one descriptor after routing all durable ACKs") {
    ...
    check_equal(flowie_cluster_publish_router_handle(router, &ack), TURBO_OK);
    ...
    check_equal(flowie_cluster_publish_router_destroy(router), TURBO_OK);
  }
}
