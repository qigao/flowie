...
static flowie_cluster_owner_directory_t *owner_directory_create(uint32_t shard_count) {
  ...
  check_equal(flowie_cluster_owner_directory_create(&config, &directory), TURBO_OK);
...
spec("Flowie cluster owner directory") {
  it("resolves a client through the versioned shard hash without database IO") {
    ...
    check_equal(flowie_cluster_owner_directory_replace(directory, entries, 4u, 7u), TURBO_OK);
    check_equal(flowie_cluster_shard_for_key(
                 ...
                 TURBO_OK);
    check_equal(flowie_cluster_owner_directory_resolve(directory, client_id, &owner), TURBO_OK);
  ...
  it("fails closed for expired and unassigned shards") {
    ...
    check_equal(flowie_cluster_owner_directory_replace(directory, entries, 2u, 1u), TURBO_OK);
  ...
  it("rejects stale and same-revision conflicting snapshots") {
    ...
    check_equal(flowie_cluster_owner_directory_replace(directory, entries, 2u, 5u), TURBO_OK);
    check_equal(flowie_cluster_owner_directory_replace(directory, entries, 2u, 5u), TURBO_OK);
    ...
    check_equal(flowie_cluster_owner_directory_replace(directory, entries, 2u, 4u),
                 TURBO_EBUSY);
    check_equal(flowie_cluster_owner_directory_revision(directory, &revision), TURBO_OK);
